...
def pesquisa(): ### Funçao que pesquisa as noticias das acoes
    try:
        consulta = ["HGTX3.SA","HGTX3", "HERING", "DTEX3.SA","DURATEX", "DTEX3", "LOGN3.SA","LOGN3","LOG-IN"]
        ...
        for j in range(0,len(consulta)):
            for i in  (search(consulta[j], tld="com.br", num=5, stop=5, pause=2)):
                ...
                with conexao.cursor() as cursor:
                    ...
                    verifica = cursor.execute('select noticia_descricao from noticias where noticia_descricao= "{}"').format((noticia.cleaned_text))
